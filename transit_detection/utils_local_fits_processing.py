...
def search_and_read_tess_lightcurve(target, sectors, data_dir):
    ...
    found_sectors = []
    light_curve_files = dict.fromkeys(sectors, None)
    for sector, sector_path in zip(sectors, sector_paths):
        try:
            ...
            light_curve_files.update({sector: lcf})
