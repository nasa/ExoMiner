...
def retrieve_shard_feature_img_pixels(
    src_tfrec_fp, src_aux_tbl_fp, feature_names, max_examples_per_tce
):
    """
    ...
    logger.info(
        f"Beginning data processing for {str(src_tfrec_fp).split('/')[-1]} using max of {max_examples_per_tce} examples per tce."
    )
    ...
    for str_record in src_tfrec_dataset:
        ...
        if tce_uid in uids_to_process_set:
            ...
            for feature_img in shard_feature_img_pixels.keys():
                logger.info(
                    f"Processing feature: {feature_img} for example #{str(record_num).zfill(4)}"
                )
                # load img feature
                feature_img_pixels = tf.reshape(
                    tf.io.parse_tensor(
                        example.features.feature[feature_img].bytes_list.value[0],
                        tf.float32,
                    ),
                    [-1],
                ).numpy()  # flattened pixels in feature img

                shard_feature_img_pixels[feature_img].extend(
                    feature_img_pixels.tolist()
                )

    logger.info(
        f"Successfully finished processing {str(src_tfrec_fp).split('/')[-1]} for a total of {recorded_record_num}/{record_num} examples."
    )
...
if __name__ == "__main__":
    ...
    train_set_tfrec_dir = Path(
        "/nobackup/jochoa4/work_dir/data/datasets/TESS_exoplanet_dataset_11-12-2024_split/tfrecords/train"
    )

    # src directory containing set aux_tbls
    src_aux_tbl_dir = Path(
        "/nobackup/jochoa4/work_dir/data/datasets/TESS_exoplanet_dataset_11-12-2024_split/tfrecords/aux_tbls"
    )

    # TRUE RUNS
    # destination directory for computed training stats
    dest_stats_dir = Path(
        "/nobackup/jochoa4/work_dir/data/stats/TESS_exoplanet_dataset_11-25-2024_split_v4/"
    )

    # destination directory for logging
    log_dir = Path(
        "/nobackup/jochoa4/work_dir/data/logging/compute_train_stats_logs_v4"
    )
    ...
    train_set_feature_img_pixels = {
        feature_img: [] for feature_img in train_set_feature_imgs
    }

    # define partial func to predefine default values for directories and target sets
    partial_func = partial(
        retrieve_shard_feature_img_pixels,
    ...
    logger.info(
        f"Beginning processing {len(jobs)} shards from range {jobs[0]} to {jobs[-1]}"
    )
    ...
    compute_and_write_example_stats(
        set_feature_img_pixels_dict=train_set_feature_img_pixels,
        dest_stats_dir=dest_stats_dir,
    )
    logger.info(f"Finished computing train_set statistics.")

    end = time.time()
    logger.info(
        f"Succesfully finished processing train_set shards in {end-start} seconds"
    )
