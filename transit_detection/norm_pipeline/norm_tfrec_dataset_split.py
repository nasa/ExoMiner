        ...
        with tf.io.TFRecordWriter(path=str(dest_tfrec_fp)) as writer:
            ...
            for string_record in src_tfrecord_dataset.as_numpy_iterator():
                ...
                for img_feature in ['diff_img', 'oot_img', 'snr_img']:
                    ...
                    norm_stats = np.load(src_norm_stats_dir / f"train_set_{img_feature}_stats.npy", allow_pickle=True).item()
...
if __name__ == "__main__":
    ...
    src_norm_stats_dir = Path('/nobackupp27/jochoa4/work_dir/data/stats/TESS_exoplanet_dataset_11-25-2024_split_v3')
    ...
    for shard_num in shards_to_process:
        ...
        SHARD_TITLE = SPLIT_NAME + '_shard_' + str(shard_num).zfill(4) + '-' + str(8611).zfill(4)  
        ...
        result = pool.apply_async(normalize_and_write_shard, args=(src_tfrec_fp, dest_tfrec_fp, src_norm_stats_dir))
