...
def normalize_and_write_shard(src_tfrec_fp, dest_tfrec_fp, dest_norm_stats_dir):
    if not src_tfrec_fp.exists():
        print("FILEPATH DOES NOT EXIST")
        print(src_tfrec_fp.name)
        return

    print(f"\nSRC: {src_tfrec_fp}\nDEST: {dest_tfrec_fp}\nSTATS: {dest_norm_stats_dir}\n")
    ...
    try:
        with tf.io.TFRecordWriter(path=str(dest_tfrec_fp)) as writer:
            ...
            for string_record in src_tfrecord_dataset.as_numpy_iterator():
                ...
                example_flux = np.array(example.features.feature['flux'].float_list.value)
                
                
                example_norm_flux = normalize_flux(example_flux)
                if example_count == 1:
                    print(f"\nFLUX: {example_flux}\nFLUX_SHAPE: {example_flux.shape}\n")
                    print(f"\nNORM_FLUX: {example_norm_flux}\nNORM_FLUX_SHAPE: {example_norm_flux}\n")
                ...
                for img_feature in ['diff_img', 'oot_img', 'snr_img']:
                    ...
                    norm_stats = np.load(dest_norm_stats_dir / f"train_set_{img_feature}_stats.npy", allow_pickle=True).item()
                    ...
                    if example_count == 1:
                        print(f"TRAIN SET MED: {img_feature_set_med} and STD: {img_feature_set_std}")
                        print(f"{img_feature}_FEATURE: {example_img_feature}")
                        print(f"NORM_{img_feature}_FEATURE: {example_norm_img_feature}")
...
if __name__ == "__main__":
    ...
    SPLIT_NAME = "test" #TODO: switch back to train split

    # Define logging directory
    log_dir = Path("/Users/jochoa4/Desktop/test_runs/norm_test_run/logging")
    log_dir.mkdir(parents=True, exist_ok=True)

    # # Define source dataset directory & dest dataset directory with same structure + norm suffix
    # src_dataset_dir = Path('/nobackupp27/jochoa4/work_dir/data/datasets/TESS_exoplanet_dataset_11-12-2024_split')
    # TEST:
    src_dataset_dir = Path('/Users/jochoa4/Desktop/test_runs/norm_test_run/dataset_1_split')
    ...
    # dest_norm_stats_dir = Path('/nobackup/jochoa4/work_dir/data/stats') / dest_dataset_dir.name / SPLIT_NAME
    #TEST: 
    dest_norm_stats_dir = Path('/Users/jochoa4/Desktop/test_runs/norm_test_run/stats') / dest_dataset_dir.name / SPLIT_NAME
    dest_norm_stats_dir.mkdir(parents=True, exist_ok=True)

    # Define pool for multiprocessing
    pool = multiprocessing.Pool(processes=1) #TODO: move up processes
    ...
    # Define shard format
    SHARD_PREFIX = SPLIT_NAME + '_shard_'
    NUM_SHARDS = 16

    # Create list of shard_nums to process from 1 to NUM_SHARDS bound
    shards_to_process = [shard_num for shard_num in range(1, NUM_SHARDS + 1)]
    ...
    for shard_num in shards_to_process:
        ...
        SHARD_TITLE = SHARD_PREFIX + str(shard_num).zfill(4) + '-' + str(NUM_SHARDS).zfill(4)  
        ...
        result = pool.apply_async(normalize_and_write_shard, args=(src_tfrec_fp, dest_tfrec_fp, dest_norm_stats_dir))
