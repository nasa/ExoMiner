...
class ExoMiner_TESS_Transit_Detection(object):
    ...
    def build_flux_window_branch(self):
        ...
        weight_initializer = tf.keras.initializers.he_normal() if self.config['global_branch_settings']['weight_initializer'] == 'he' \
            else 'glorot_uniform'

        # initialize branch
        branch_name = 'flux_window'
        view_inputs = self.inputs[self.config['flux_window_branch_settings']['window'][0]]
        view_inputs = self.log_input_per_batch(f"flux_window: {self.inputs[self.config['flux_window_branch_settings']['window'][0]]}")(view_inputs)
        ...
        init_conv_filters = self.config['flux_window_branch_settings']['init_conv_filters']
        conv_ls_per_block = self.config['flux_window_branch_settings']['conv_ls_per_block'] # self.config['conv_ls_per_block']
        ...
        for conv_block_i in range(n_blocks):  # create convolutional blocks
            ...
            for seq_conv_block_i in range(conv_ls_per_block):  # create convolutional layers for the block
                net = tf.keras.layers.Conv1D(dilation_rate=1,
                                             ...
                                             name='conv{}_{}_{}'.format(branch_name, conv_block_i, seq_conv_block_i),
                ...
                if self.config['global_branch_settings']['non_lin_fn'] == 'lrelu':
                    net = tf.keras.layers.LeakyReLU(alpha=0.01,
                                                    name='lrelu{}_{}_{}'.format(branch_name, conv_block_i,
                                                                                seq_conv_block_i))(net)
                elif self.config['global_branch_settings']['non_lin_fn'] == 'relu':
                    net = tf.keras.layers.ReLU(name='relu{}_{}_{}'.format(branch_name, conv_block_i,
                                                                          seq_conv_block_i))(net)
                    ...
                    net = tf.keras.layers.PReLU(alpha_initializer='zeros',
                                                ...
                                                shared_axes=[1, 2],
                                                name='prelu{}_{}_{}'.format(branch_name, conv_block_i,
                                                                            seq_conv_block_i))(net)

            net = tf.keras.layers.MaxPooling1D(pool_size=pool_size,
                                               strides=pool_stride,
                                               name='maxpooling_{}_{}'.format(branch_name, conv_block_i))(net)

        # flatten output of the final conv block (None, 12, 16) -> (None, 192)
        net = tf.keras.layers.Flatten(data_format='channels_last', name='flatten_{}'.format(branch_name))(net)

        if self.config['global_branch_settings']['num_fc_conv_units'] > 0:
            net = tf.keras.layers.Dense(units=self.config['global_branch_settings']['num_fc_conv_units'],
                                        kernel_regularizer=None,
                                        activation=None,
                                        use_bias=True,
                                        ...
                                        kernel_constraint=None,
                                        bias_constraint=None,
                                        name='fc_{}'.format(branch_name))(net)

            if self.config['global_branch_settings']['non_lin_fn'] == 'lrelu':
                net = tf.keras.layers.LeakyReLU(alpha=0.01, name='fc_lrelu_{}'.format(branch_name))(net)
            elif self.config['global_branch_settings']['non_lin_fn'] == 'relu':
    ...
    def build_diff_img_branch(self):
        ...
        weight_initializer = tf.keras.initializers.he_normal() \
            if self.config['global_branch_settings']['weight_initializer'] == 'he' else 'glorot_uniform'
        
        branch_name = 'diff_img'

        branch_view_inputs = [self.inputs[view_name] for view_name in self.config['diff_img_branch_settings']['imgs']] # inital shape per input is (None, 33, 33) 
        branch_view_inputs = self.log_input_per_batch(f"diff_imgs: {[self.inputs[view_name] for view_name in self.config['diff_img_branch_settings']['imgs']]}")(branch_view_inputs)
        print(f"Searching for branch_view_inputs for build_diff_img_branch: {branch_view_inputs}")
        branch_view_inputs = [tf.expand_dims(l, axis=-1, name='expanding_diff_img') for l in branch_view_inputs] # [(None, 33, 33, 1) ..]
        branch_view_inputs = tf.keras.layers.Concatenate(axis=3, name='input_diff_img_concat')(branch_view_inputs) # (None, 33, 33, 4)
        ...
        net = tf.keras.layers.Flatten(data_format='channels_last', name='flatten_convfc_{}'.format(branch_name))(net)
        ...
        if self.config['global_branch_settings']['num_fc_conv_units'] > 0:
            ...
            net = tf.keras.layers.Dropout(self.config['global_branch_settings']['dropout_rate_fc_conv'], name='dropout_fc_{}'.format(branch_name))(net)
    ...
    def build_fc_block(self, net):
        ...
        for fc_layer_i in range(self.config['fc_block_settings']['num_fc_layers']):
            ...
            if self.config['fc_block_settings']['decay_rate'] is not None:
                net = tf.keras.layers.Dense(units=fc_neurons,
                                            kernel_regularizer=regularizers.l2(
                                                self.config['fc_block_settings']['decay_rate']) if self.config['fc_block_settings'][
                                                                                  'decay_rate'] is not None else None,
                                            activation=None,
                                            use_bias=True,
                                            kernel_initializer=self.config['global_branch_settings']['kernel_initializer'],
                                            bias_initializer=self.config['global_branch_settings']['bias_initializer'],
                                            bias_regularizer=None,
                                            activity_regularizer=None,
                                            kernel_constraint=None,
                                            bias_constraint=None,
            ...
            net = tf.keras.layers.Dropout(self.config['fc_block_settings']['dropout_rate'], name=f'dropout_fc{fc_layer_i}')(net)

        return net
    
    def log_input_per_batch(self, name):
        def _log_tensor(t):
            tf.print(f"[Batch] {name} shape: ", tf.shape(t), " | sample:", t[0][:4])
            return t
        return Lambda(_log_tensor)
