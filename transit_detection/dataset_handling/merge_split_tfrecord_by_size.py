    ...
    for split in splits:
        ...
        for _, row in split_df.iterrows():
            ...
            if examples >= min_examples:
                ...
                temp_df = pd.DataFrame({"resized_shard" : [merged_filename], "original_shard": [tfrec_fn], "resized_examples" : [examples]})
            ...
            if current_count + examples < min_examples:
                ...
                if current_count + examples > min_examples + max_extra:
                    ...
                    for orig_shard, count in current_group:
                        temp_df = pd.DataFrame({"resized_shard" : [merged_filename], "original_shard": [orig_shard], "resized_examples" : [current_count]})
                        merge_map_df = pd.concat([merge_map_df, temp_df], ignore_index=True)
                    # start new group    
                    shard_index += 1
