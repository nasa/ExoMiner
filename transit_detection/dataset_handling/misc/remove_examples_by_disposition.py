...
def process_shard(shard_num, input_dir, output_dir, remove_dispositions):
    try:
        ...
        for split_name in ["train", "test", "val"]:
            input_tfrec_fp = (
                input_dir
                / split_name
                / f"norm_{split_name}_shard_{shard_num}-8611.tfrecord"
            )
            ...
            output_tfrec_fp = (
                output_tfrec_dir / f"norm_{split_name}_shard_{shard_num}-8611.tfrecord"
            )
...
if __name__ == "__main__":
    ...
    # Dispositions to remove
    remove_dispositions = ["NTP"]

    # src directory for raw tfrecord shards
    src_tfrec_dir = Path(
        "/nobackupp27/jochoa4/work_dir/data/datasets/TESS_exoplanet_dataset_06-20-2025_split_norm/tfrecords"
    )
    assert (
    ...
    dest_tfrec_dir = Path(
        "/nobackupp27/jochoa4/work_dir/data/datasets/TESS_exoplanet_dataset_06-20-2025_split_norm_non_ntp/tfrecords"
    )
    dest_tfrec_dir.mkdir(parents=True, exist_ok=True)

    # destination directory for logging
    log_dir = Path(
        "/nobackupp27/jochoa4/work_dir/data/logging/remove_ntp_TESS_exoplanet_dataset_06-20-2025_split_norm"
    )
    ...
    partial_func = partial(
        ...
        remove_dispositions=remove_dispositions,
    )
    start = time.time()
