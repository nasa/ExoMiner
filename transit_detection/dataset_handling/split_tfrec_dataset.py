...
def process_shard(input_shard_num, input_dir, output_dir, train_targets, val_targets, test_targets):
    try:
        ...
        val_tfrec_fp = val_tfrec_dir/ f"val_shard_{input_shard_num}-8611"
        test_tfrec_fp = test_tfrec_dir / f"test_shard_{input_shard_num}-8611"
        ...
        print(f"COMPLETED processing tfrecord {str(input_shard_fp).split('/')[-1]} successfully")

        return f"COMPLETED processing tfrecord {str(input_shard_fp).split('/')[-1]} successfully with {train_count} train ex., {val_count} val ex., and {test_count} test ex."

    except Exception as e:
        print(f"ERROR while processing tfrecord {str(input_shard_fp).split('/')[-1]}: {e}")
        return f"ERROR while processing tfrecord {str(input_shard_fp).split('/')[-1]}: {e}"


if __name__ == "__main__":
    # load merged aux tbl
    aux_tbl = pd.read_csv("/nobackup/jochoa4/work_dir/data/datasets/TESS_exoplanet_dataset_11-12-2024/merged_tfrecords/merged_data_tbl_chunks_0001-8611.csv")
    ...
    dest_tfrec_dir = Path("/nobackup/jochoa4/work_dir/data/datasets/TESS_exoplanet_dataset_11-12-2024_split/tfrecords")
    dest_tfrec_dir.mkdir(parents=True, exist_ok=True)

    # destination directory for logging
    log_dir = Path("/nobackup/jochoa4/work_dir/data/logging/split_dataset_logs") #TODO: revisit and update dest_log_dir
    ...
    logger.info(f'Beginning processing {len(jobs)} chunks from range {jobs[0]} to {jobs[-1]}')
    ...
    print(f'Finished processing train val test split using 128 shards in {end - start} seconds')
