...
# local
from transit_detection.utils_flux_processing import extract_flux_windows_for_tce
...
data_dir = Path('/Users/msaragoc/Downloads/transit_detection/data/test_create_dataset_5-31-2024_1017')
# set light curve data directory
lc_dir = Path('/Users/msaragoc/Downloads/transit_detection/data/lc_data/')
# set target pixel file data directory
tpf_dir = Path('/Users/msaragoc/Downloads/transit_detection/data/tpf_data/')
# TCE table
...
data_to_tfrec = []
for tce_i, tce_data in tce_tbl.iterrows():  # iterate over each TCE
    ...
    print(f'Extracting and processing data for TCE {tce_data["uid"]}...')

    # create directory for TCE plots
    plot_dir_tce = plot_dir / f'tce_{tce_data["uid"]}'
    plot_dir_tce.mkdir(exist_ok=True, parents=True)
    ...
    for sector_i, sector in enumerate(found_sectors):  # iterate over sectors
        ...
        plot_dir_tce_sector = plot_dir_tce / f'sector_{sector}'
        plot_dir_tce_sector.mkdir(exist_ok=True, parents=True)
        ...
        # print(search_lc_res)
        # # download light curve fits file(s)
        # lcf = search_lc_res.download_all(download_dir=str(lc_dir), quality_bitmask='default', flux_column='pdcsap_flux')
        # lcf = lcf.stitch(lambda x: lk.LightCurve(time=x.time, flux=np.array(x.flux)))

        # with fits.open(lcf.filename, mode="readonly") as hdulist:
        #
        #     time = hdulist[1].data['TIME']
        #     pdcsap_flux = hdulist[1].data['PDCSAP_FLUX']
        #     dq_flags = hdulist[1].data['QUALITY']
        ...
        (resampled_flux_it_windows_arr, resampled_flux_oot_windows_arr, midtransit_points_windows_arr,
         midoot_points_windows_arr) = extract_flux_windows_for_tce(time, flux, tce_time0bk, period_days, tce_duration,
        ...
        plot_dir_tce_diff_img_data = plot_dir_tce_sector / 'diff_img_data'
