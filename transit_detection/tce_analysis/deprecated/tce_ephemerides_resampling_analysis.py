...
def analyze_tce_sampling(tce_time0bk, tce_period, tce_duration, resampling_rates, n_durations_window=5):#, num_transits_in_window = 3):
    ...
    tce_duration = tce_duration / 24 # scale to days
    ...
    time_it_cadence_arr = np.zeros(len(time_arr))
    ...
    return n_resampled_it_points, n_transits_in_window


def build_and_save_plots(df, plot_dir, resampling_rate, per_observation):
    # Joint Plots & Save
    palette = "colorblind"
    ...
    sns.violinplot(data=df, x='tce_label', y=f'resampled_it_points_per_{plot_type}_{resampling_rate}', hue='tce_label', linewidth=1)
    plt.ylim(0,None)
    plt.title(f'Violin Plot of Estimated In-Transit Point Count per TCE {plot_type} by Disposition')
    plt.xlabel('TCE Label')
    plt.ylabel(f'Estimated In-Transit Points per TCE {plot_type}')
    plt.tight_layout()
    plt.savefig(plot_dir / f"violin_plot_all_dispositions_per_{plot_type}_{resampling_rate}.png", format="png", dpi=300, bbox_inches="tight")
    plt.close()

    # 2. Hist Plot for all Dispositions
    plt.figure(figsize=(10,6))
    sns.set_palette(palette)
    sns.displot(data=df, x=f'resampled_it_points_per_{plot_type}_{resampling_rate}', hue='tce_label', bins=discrete_bins, element="step")
    plt.title(f'Distribution of Estimated In-Transit Points per TCE {plot_type} (Full Range)')
    plt.xlabel(f'Estimated In-Transit Points per TCE {plot_type}')
    plt.ylabel('Frequency')
    plt.tight_layout()
    plt.savefig(plot_dir / f"hist_plot_all_dispositions_per_{plot_type}_{resampling_rate}.png", format="png", dpi=300, bbox_inches="tight")
    plt.close()
    
    # 3. Normalized Hist Plot for all Dispositions
    plt.figure(figsize=(10,6))
    sns.set_palette(palette)
    sns.displot(data=df, x=f'resampled_it_points_per_{plot_type}_{resampling_rate}', hue='tce_label', stat="density", common_norm=False, bins=discrete_bins, element="step") #linewidth=1)
    plt.title(f'Normalized Distribution of Estimated In-Transit Points per TCE {plot_type} (Full Range)')
    plt.xlabel(f'Estimated In-Transit Points per TCE {plot_type}')
    plt.ylabel('Probability Density of TCEs')
    plt.tight_layout()
    plt.savefig(plot_dir / f"hist_plot_all_dispositions_normalized_per_{plot_type}_{resampling_rate}.png", format="png", dpi=300, bbox_inches="tight")
    plt.close()

    # 4. Log Scale Hist Plot for all Dispositions
    plt.figure(figsize=(10,6))
    sns.set_palette(palette)
    sns.displot(data=df, x=f'resampled_it_points_per_{plot_type}_{resampling_rate}', hue='tce_label', log_scale=(True,False), stat="count", element="step")#linewidth=1)
    plt.title(f'Log-Scaled Distribution of Estimated In-Transit Points per TCE {plot_type} (Full Range)')
    plt.xlabel(f'Log(Estimated In-Transit Points per TCE {plot_type})')
    plt.ylabel('Frequency')
    plt.tight_layout()
    plt.savefig(plot_dir / f"hist_plot_all_dispositions_log_scale_per_{plot_type}_{resampling_rate}.png", format="png", dpi=300, bbox_inches="tight")
    plt.close()

    # 5. Hist Plot for 0-10 for all Dispositions
    plt.figure(figsize=(10,6))
    sns.set_palette(palette)
    sns.displot(data=df, x=f'resampled_it_points_per_{plot_type}_{resampling_rate}', hue='tce_label', bins=list(range(11)), binrange=(0,10), element="step",)
    plt.title(f'Distribution of Estimated In-Transit Points per TCE {plot_type} (0-10 Range)')
    plt.xlabel(f'Estimated In-Transit Points per TCE {plot_type}')
    plt.ylabel('Frequency')
    plt.tight_layout()
    plt.savefig(plot_dir / f"hist_plot_all_dispositions_0-10_per_{plot_type}_{resampling_rate}.png", format="png", dpi=300, bbox_inches="tight")
    plt.close()

    # 6. Normalized Hist Plot for 0-10 for all Dispositions
    plt.figure(figsize=(10,6))
    sns.set_palette(palette)
    sns.displot(data=df, x=f'resampled_it_points_per_{plot_type}_{resampling_rate}', hue='tce_label', stat="density", common_norm=False, bins=list(range(11)), binrange=(0,10), element='step')
    plt.title(f'Normalized Distribution of Estimated In-Transit Points per TCE {plot_type} (0-10 Range)')
    plt.xlabel(f'Estimated In-Transit Points per TCE {plot_type}')
    plt.ylabel('Probability Density of TCEs')
    plt.tight_layout()
    plt.savefig(plot_dir / f"hist_plot_all_dispositions_0-10_normalized_per_{plot_type}_{resampling_rate}.png", format="png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    for label_i, label in enumerate(df['tce_label'].unique()):
        ...
        plt.figure(figsize=(10,6))
        sns.displot(data=df.loc[df['tce_label'] == label], x=f'resampled_it_points_per_{plot_type}_{resampling_rate}', color=sns.color_palette(palette,6)[label_i], bins=discrete_bins)
        plt.gca().yaxis.set_major_locator(MaxNLocator(integer=True)) #set y axis to display as int for small frequencies
        plt.title(f'Distribution of Estimated In-Transit Points per TCE {plot_type} for {label}\'s')
        plt.xlabel(f'Estimated In-Transit Points per TCE {plot_type}')
        plt.ylabel('Frequency')
        plt.tight_layout()
        plt.savefig(plot_dir / f"hist_plot_disposition_{label}_per_{plot_type}_{resampling_rate}.png", format="png", dpi=300, bbox_inches="tight")
        plt.close()
...
if __name__ == '__main__':
    ...
    save_path = Path('/Users/jochoa4/Downloads/transit_detection/data/tce_analysis/tce_ephemerides_analysis_v2')
    ...
    for tce_i, tce_data in tce_tbl.iterrows(): # for all tces associated with target
        ...
        n_resampled_it_points, n_transits_in_window = analyze_tce_sampling(tce_time0bk, tce_period, tce_duration, resampling_rates, n_durations_window)

        tce_data[f'n_transits_in_window'] = n_transits_in_window

        for resampling_rate, n_resampled_it_points in zip(resampling_rates, n_resampled_it_points):
            tce_data[f'resampled_it_points_per_window_{resampling_rate}'] = n_resampled_it_points
            tce_data[f'resampled_it_points_per_observation_{resampling_rate}'] = n_resampled_it_points / n_transits_in_window
    ...
    for resampling_rate in resampling_rates:
        ...
        summary_stats.to_csv( ss_save_path / f'summary_statistics_per_observation_{resampling_rate}.csv' )


        summary_stats = df.groupby('tce_label')[f'resampled_it_points_per_window_{resampling_rate}'].agg(['min','max','std','mean','median','count'])
        cumulative_stats = pd.DataFrame({
            'min' : [df[f'resampled_it_points_per_window_{resampling_rate}'].min()],
            'max' : [df[f'resampled_it_points_per_window_{resampling_rate}'].max()],
            'std' : [df[f'resampled_it_points_per_window_{resampling_rate}'].std()],
            'mean' : [df[f'resampled_it_points_per_window_{resampling_rate}'].mean()],
            'median' : [df[f'resampled_it_points_per_window_{resampling_rate}'].median()],
            'count' : [df[f'resampled_it_points_per_window_{resampling_rate}'].count()]
        }, index=['Overall'])

        summary_stats.to_csv( ss_save_path / f'summary_statistics_per_window_{resampling_rate}.csv' )
    ...
    for resampling_rate in resampling_rates:
        ...
        for plot_range in ('per_observation', 'per_window'):
            range_plot_path = plot_dir / f'{plot_range}'
            range_plot_path.mkdir(exist_ok=True, parents=True)
            print(range_plot_path)

            if plot_range == 'per_observation':
                build_and_save_plots(df, range_plot_path, resampling_rate, per_observation=True)
            else:
                build_and_save_plots(df, range_plot_path, resampling_rate, per_observation=False)
