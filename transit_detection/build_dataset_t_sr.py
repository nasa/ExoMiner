...
def process_target_sector_run(
    ...
    logger.info(
        f"With chunked_target_sector_run_data of len: {len(chunked_target_sector_run_data)}"
    )
    logger.info(f"Head: {chunked_target_sector_run_data.head()}")
    ...
    for target, sector_run, t_sr_tce_data in chunked_target_sector_run_data:
        ...
        try:
            ...
            logger.info(f"Using t_sr_tce_data: {t_sr_tce_data}")
            ...
            for tce_i, tce_data in t_sr_tce_data.iterrows():
                ...
                logger.info(f"tce_data form: {tce_data[interest_cols]}")
            ...
            for sector, lcf, tpf in zip(found_sectors, found_lcfs, found_tpfs):
                logger.info(f"Beginning processing for S{sector} in SR{sector_run}")
                ...
                logger.info(
                    f"T_{target}, SR_{sector_run}, S{sector} has lcf with min/max flux: {min(flux), max(flux)} of shape {flux}"
                )
                logger.info(
                    f"T_{target}, SR_{sector_run}, S{sector} has lcf with min/max flux (excl nans): {min(flux[np.isfinite(flux)]),max(flux[np.isfinite(flux)])}"
                )
                logger.info(
                    f"T_{target}, SR_{sector_run}, S{sector} has lcf with {sum(~np.isfinite(flux))} nans"
                )
                ...
                logger.info(
                    f"T_{target}, SR_{sector_run}, S{sector} has lcf with min/max interp flux: {min(flux), max(flux)}"
                )
                logger.info(
                    f"T_{target}, SR_{sector_run}, S{sector} has cadence_mask with {sum(cadence_mask)} / {len(cadence_mask)} masked vals"
                )
                logger.info(
                    f"T_{target}, SR_{sector_run}, S{sector} has flux_quality with {len(flux_quality[flux_quality < 1.0])} / {len(cadence_mask)} masked vals"
                )
                ...
                logger.info(
                    f"T_{target}, SR_{sector_run}, S{sector} has detrended_lcf with min/max interp flux: {min(flux), max(flux)} of shape {flux}"
                )
                # if t_sr_s_plot_dir: #TODO: revert
                # plot_detrended_flux_time_series_sg(
                #     time=lcf.time,
                #     flux=lcf.flux,
                #     detrend_time=time,
                ...
                # )
                logger.info(
                    f"T_{target}, SR_{sector_run}, S{sector}: {target} min/max trend: {trend.min(), trend.max()}"
                )
                # if str(target) == "444827931":
                ...
                for tce_i, tce_data in t_sr_tce_data.iterrows():
                    logger.info(
                        f"Processing tce_i: {tce_i} using tce_data: {tce_data[interest_cols]}"
                    )
...
if __name__ == "__main__":
    ...
    logger.info(f"Filtering TCE table by disposition")
