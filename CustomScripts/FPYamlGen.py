...
"""
Generate the filepaths to the train, test, and validation shards with Cross-validation. Done after the cross-validation shards are generated.
"""
...
for i, paths in enumerate(cv_iterations):
    ...
    cv_iter['test'] = [Path(f"/Users/agiri1/Desktop/ExoBD_Datasets/shard_tables/cv_iter_{i}/norm_data/{path.parts[-1]}") for path in paths['test']]
    cv_iter['val'] = [Path(f"/Users/agiri1/Desktop/ExoBD_Datasets/shard_tables/cv_iter_{i}/norm_data/{path.parts[-1]}") for path in paths['val']]

    no_test_fps = [Path(f"/Users/agiri1/Desktop/ExoBD_Datasets/shard_tables/cv_iter_{i}/norm_data/{path.parts[-1]}") for path in paths['train']]
