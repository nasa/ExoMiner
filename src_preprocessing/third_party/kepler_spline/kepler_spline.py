...
from src_preprocessing.third_party.robust_mean import robust_mean
...
def choose_kepler_spline(all_time,
  ...
  divided into different segments (e.g. split by quarter breaks or gaps in the data).
  A separate spline is fit for each segment.
  ...
  Args:
    ...
    outlier_cut: float, the maximum number of standard deviations from the median spline residual before a point is
  ...
  Returns:
    spline: List of numpy arrays; values of the best-fit spline corresponding to the input flux arrays.
  ...
  """
    ...
    for bkspace in bkspaces:
        ...
        for time, flux in zip(all_time, all_flux):
            ...
            try:
                spline_piece, mask = kepler_spline(time, flux, bkspace=bkspace, maxiter=maxiter,
...
def fit_kepler_spline(all_time,
  ...
  Returns:
    ...
    metadata: Object containing metadata about the spline fit.
  """
    # Logarithmically sample bkspace_num candidate break point spacings between bkspace_min and bkspace_max.
