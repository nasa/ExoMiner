...
The first table should also have columns 'sector_run' and 'sectors_observed':
- sector_run: in format <start_sector>-<end_sector> for multisector run; simply <sector> for single-sector runs
- sectors_observed: in format <s_i>_<s_j>... (e.g., '95_96', '3'). These are the sectors in which the target was observed for the corresponding sector run.

The table with the start and end timestamps for each sector must contain columns 'sector', 'start_btjd', 'end_btjd'. 
"""
...
def compute_correlation_coeff(bin_ts_a, bin_ts_b):
    ...
    norm_bin_ts_a = np.linalg.norm(bin_ts_a)
    norm_bin_ts_b = np.linalg.norm(bin_ts_b)
    
    if norm_bin_ts_a == 0 or norm_bin_ts_b == 0:
        corr_coef = np.nan
    else:
        corr_coef = np.sum(bin_ts_a * bin_ts_b) / (norm_bin_ts_a * norm_bin_ts_b)
...
def match_transit_signals(transit_signal_a, transit_signal_b, sampling_interval, tstart, tend, plot_bin_ts=False,
    ...
    Args:
        ...
        tstart: float/float list, start time(s) for binary time series
        tend: float/float list, end time(s) for binary time series
    ...
    if isinstance(tend, float):
        tend = np.array([tend])

    transit_signal_a_bin_ts_lst, transit_signal_b_bin_ts_lst = [], []
    for tstart_val, tend_val in zip(tstart, tend):
        transit_signal_a_bin_ts = create_binary_time_series(
            epoch=find_first_epoch_after_this_time(transit_signal_a['epoch'], transit_signal_a['period'], tstart_val),
        ...
        transit_signal_a_bin_ts_lst.append(transit_signal_a_bin_ts)

        epoch_new = find_first_epoch_after_this_time(transit_signal_b['epoch'], transit_signal_b['period'], tstart_val)
        ...
        transit_signal_b_bin_ts_lst.append(transit_signal_b_bin_ts)

    transit_signal_a_bin_ts = np.concatenate(transit_signal_a_bin_ts_lst)
    transit_signal_b_bin_ts = np.concatenate(transit_signal_b_bin_ts_lst)
    
    corr_coeff = compute_correlation_coeff(transit_signal_a_bin_ts, transit_signal_b_bin_ts)

    if plot_bin_ts:

        time_arr = np.concatenate([np.linspace(tstart_val, tend_val, int((tend_val - tstart_val) / sampling_interval), endpoint=True) 
                                   for tstart_val, tend_val in zip(tstart, tend)])
...
def match_transit_signals_in_target(targets_arr, tce_tbl, objects_tbl, sector_timestamps_tbl, sampling_interval,
    ...
    Args:
        ...
        sector_timestamps_tbl: pandas DataFrame, table with start and end timestamps for each sector
        sampling_interval: float, sampling interval used to create the binary time series
    ...
    for target_i, target in enumerate(targets_arr):
        ...
        for sector_run_i, sector_run in enumerate(sector_runs_target):
            ...
            # get star and end timestamps for the sectors the TIC was observed
            target_sectors_obs = tces_in_tic_sectorun['sectors_observed'][0]  # they are the same for all TCEs in same sector run
            if '_' in target_sectors_obs:
                target_sectors_obs = [int(el) for el in target_sectors_obs.split('_')]
            else:
                target_sectors_obs = [int(target_sectors_obs)]
                
            # get start and end timestamps for sectors where target was observed
            target_sectors_obs_timestamps = sector_timestamps_tbl.loc[sector_timestamps_tbl['sector'].isin(target_sectors_obs)]
            if len(target_sectors_obs_timestamps) == 0:
                raise ValueError(f'Sector(s) {target_sectors_obs} were not found in sectors timestamps table.')
            tstart, tend = target_sectors_obs_timestamps[['start_btjd', 'end_btjd']].T.to_numpy()
