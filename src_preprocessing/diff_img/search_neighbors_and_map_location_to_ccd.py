...
def map_star_celestial_coordinates_to_ccd(stars_tbl, sector=None):
    ...
    # initialize scinfo
    _, _, _, _, _, _, _, _, scinfo = tess_stars2px_function_entry(stars_tbl['ID'][0], stars_tbl['ra'][0], stars_tbl['dec'][0], trySector=sector)

    stars_ids, _, _, _, _, _, outColPix, outRowPix, scinfo = (
        tess_stars2px_function_entry(stars_tbl['ID'], stars_tbl['ra'], stars_tbl['dec'], scInfo=scinfo, trySector=sector))
...
def create_neighbors_table(neighbors_tbl, save_fp, sector=None, logger=None):
    ...
    stars_px_coords_tbl = map_star_celestial_coordinates_to_ccd(neighbors, sector)
...
if __name__ == "__main__":
    ...
    mag_thr = np.inf
    # results directory
    res_dir = Path(f'/home/msaragoc/Projects/exoplnt_dl/experiments/search_neighboring_stars/tess_spoc_2min_s1-s68_search_radius_arcsec_{search_radius_arcsec.value}_mag_thr_{mag_thr}_1-30-2025_1212')
