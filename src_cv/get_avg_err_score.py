...
Recall/Precision Theshold:
Recall/Precision thresold at x means that any examples where the score is between 0.5-x and 0.5+x
are not included in calculating either metric
'''
...
for label in labels:
    ...
    wrong_scores = ranking_tbl.loc[(ranking_tbl['predicted class'] != ranking_tbl['label']) &
                                   (ranking_tbl['original_label'] == label)]['score']
    ...
    for thresh in [0] + non_class_thresholds:
        ...
        total_label = ranking_tbl.loc[(ranking_tbl['original_label'] == label) &
        ...
        total_correct = ranking_tbl.loc[(ranking_tbl['predicted class'] == ranking_tbl['label']) &
                                        (ranking_tbl['original_label'] == label) &
                                        ((ranking_tbl['score'] < class_threshold - thresh) |
                                         (ranking_tbl['score'] > class_threshold + thresh))].shape[0]
...
for thresh in non_class_thresholds:
    ...
    total_1 = ranking_tbl.loc[(ranking_tbl['label'] == 1) &
                                  ((ranking_tbl['score'] < class_threshold - thresh) |
                                   (ranking_tbl['score'] > class_threshold + thresh))].shape[0]
    ...
    total_correct = ranking_tbl.loc[(ranking_tbl['label'] == 1) &
                                    (ranking_tbl['score'] > class_threshold + thresh)].shape[0]
