...
CV_SCRIPT=/home/msaragoc/Projects/exoplnt_dl/codebase/src_cv/cv_predict.py
...
CONFIG_FILE=/home/msaragoc/Projects/exoplnt_dl/codebase/src_cv/config_cv_predict.yaml

# Number of total CV iterations run is min(number of folds defined in the config file, N_CV_ITERS * N_CV_ITERS_SIMULT)
N_CV_ITERS=5  # number of groups of CV iterations that are run at the same time
...
do
    echo "Running $N_CV_ITERS_SIMULT CV iterations ($(expr $cv_i + 1) out of $(expr $N_CV_ITERS))..."
    mpiexec -np $N_CV_ITERS_SIMULT python $CV_SCRIPT --job_idx=$cv_i  --config_file=$CONFIG_FILE &> "$PYOUT_DIR"cv_iter_$cv_i.txt
    echo "Finished running $N_CV_ITERS_SIMULT CV iterations ($(expr $cv_i + 1) out of $(expr $N_CV_ITERS))"
done
