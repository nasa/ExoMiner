...
from old.src_old.estimator_util_bc import InputFn, ModelFn, CNN1dModel, get_data_from_tfrecord
import old.src_old.config
...
if __name__ == "__main__":
    ...
    for config in configs:
        configs[config] = old.old.config.add_dataset_params(tfrec_dir, satellite, multi_class, centr_flag,
                                                            use_kepler_ce, configs[config])

    # add missing parameters in hpo with default values
    for config in configs:
        configs[config] = old.old.config.add_default_missing_params(config=configs[config])
