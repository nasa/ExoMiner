...
from archived_experiments.label_noise.utils_label_noise import processing_data_run, train_model, eval_ensemble


def inject_label_noise_run(experiment_dir, prob_transition_mat, run_params, logger=None):
    """ Run one iteration of label noise injection.
    ...
    for run_per_tau in range(run_params['num_runs_per_tau']):
        ...
        run_params['rng'] = np.random.default_rng(seed=run_params['rnd_seed'] * run_per_tau + run_params['run_id'])
...
def run_label_noise_experiment():
    ...
    if rank == 0:
        ...
        with open(run_params['experiment_dir'] / 'run_params.json', 'w') as run_run_file:
            json.dump(json_dict, run_run_file)
    ...
    # run_id = rank
    # logger.info(f'Running iteration {run_id} (out of {len(run_params["prob_transition_mats"])}): '
    #             f'{run_params["prob_transition_mats"][run_id]}')
    # run_params['run_id'] = run_id
    # inject_label_noise_run(
    #     run_params['experiment_dir'],
    #     run_params['prob_transition_mats'][run_id],
    #     run_params,
    #     logger
    # )
