...
def run_setup_for_cv_iter(cv_iter, cv_iter_dir, config, model_i=0):
    ...
    if config['fine_tuning_hyperparameters_fp']:
        with open(config['fine_tuning_hyperparameters_fp'], 'r') as file:
            fine_tuning_hyperparameters = yaml.unsafe_load(file)
            print(f'Updating hyperparameters for fine-tuning:\n {fine_tuning_hyperparameters}')
            # config['config'].update(fine_tuning_hyperparameters)
            config['config']['lr'] /= 10
