...
def match_tces_to_astronet_traininset(matching_tbl, astronet_tbl, sampling_interval, astronet_tbl_cols, res_dir):
    ...
    distance. For each TCE, gets objects that belong to the same TIC and computes matching distance between the TCE and
    each possible object in astronet training set.
    ...
    for tce_i, tce in matching_tbl.iterrows():
        ...
        objs_in_tic = astronet_tbl.loc[astronet_tbl['tic'] == tce['target_id']].reset_index()

        if len(objs_in_tic) == 0:
            continue

        # iterate over EBs in the same TIC
        for obj_i, obj in objs_in_tic.iterrows():

            phase = max(tce['tce_period'], obj['period'])
