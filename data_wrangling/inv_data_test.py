...
#%% Convert ascii Inverted TCE tables to csv

invTblFp = '/data5/tess_project/Data/Ephemeris_tables/Kepler/Inverted DR25/kplr_dr25_inv_tces.txt'
...
invDf = pd.read_table(invTblFp, skiprows=75, names=columnNames, skipinitialspace=False, delim_whitespace=True)

print(invDf.head())
# aaa
print(len(invDf))

invDf.to_csv(invTblFp.replace('.txt', '.csv'), index=False)

#%% Add stellar parameters to Inverted TCE tables
...
invTblFp = '/data5/tess_project/Data/Ephemeris_tables/Kepler/Inverted DR25/kplr_dr25_inv_tces.csv'

count_vec = []

print(invTblFp.split('/')[-1])

invDf = pd.read_csv(invTblFp)

for stellar_param in stellar_fields_out:
    invDf[stellar_param] = np.nan

count = 0
for row_star_i, row_star in stellar_tbl.iterrows():

    if row_star_i % 100 == 0:
        print('Star {} out of {} ({} %)\n Number of Inverted TCEs updated: {}'.format(row_star_i,
                                                                             len(stellar_tbl),
                                                                             row_star_i / len(stellar_tbl) * 100,
                                                                             count))

    target_cond = invDf['KIC'] == row_star['kepid']

    count += target_cond.sum()

    invDf.loc[target_cond, stellar_fields_out] = row_star[stellar_fields_in].values

    if count == len(invDf):
        break

count_vec.append((invTblFp, count))
print('Number of Inverted TCEs updated: {}'.format(count))
invDf.to_csv(invTblFp.replace('.csv', '_stellar.csv'), index=False)

print(count_vec)

#%% Check that stellar parameters are not NaN for any Inverted TCE
...
invTblFp = '/data5/tess_project/Data/Ephemeris_tables/Kepler/Inverted DR25/kplr_dr25_inv_tces_stellar.csv'

print(invTblFp.split('/')[-1])

invDf = pd.read_csv(invTblFp, usecols=stellar_fields_out)

print(invDf.isna().any(axis=0))
...
invDf.rename(columns=renameDict, inplace=True)

# remove TCEs with zero period or transit duration
invDf = invDf.loc[(invDf['tce_period'] > 0) & (invDf['tce_duration'] > 0)]
print(len(invDf))

invDf.to_csv(invTblFp.replace('.csv', '_processed.csv'), index=False)

#%% Add label to the Inverted TCEs

invTblFp = '/data5/tess_project/Data/Ephemeris_tables/Kepler/Inverted DR25/kplr_dr25_inv_tces_stellar_processed.csv'
...
invDf.to_csv(invTblFp.replace('.csv', '_withlabels.csv'), index=False)
