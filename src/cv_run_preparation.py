...
from data_wrangling.update_results_table import create_metric_mapping
...
def create_shard_fold(shard_tbl_fp, dest_tfrec_dir, fold_i, src_tfrec_fps):
    ...
    with tf.io.TFRecordWriter(str(tfrec_new_fp)) as writer:
        ...
        for tce_i, tce in fold_tce_tbl.iterrows():
            ...
            for src_tfrec in src_tfrec_fps:
                ...
                for string_i, string_record in enumerate(tfrecord_dataset.as_numpy_iterator()):
                    ...
                    # add TCE found to the TFRecord shard
                    if f'{target_id}-{tce_id}' != f'{tce["target_id"]}-{tce["tce_plnt_num"]}':
                        continue
                    else:
                        for feature_name, feature_info in features_to_add.items():
                            if feature_info['dtype'] == 'float':
                                example_util.set_float_feature(example, f'{feature_name}',
                                                               [tce[feature_name]],
                                                               allow_overwrite=True)
                            elif feature_info['dtype'] == 'int':
                                example_util.set_int64_feature(example, f'{feature_name}',
                                                               [tce[feature_name]],
                                                               allow_overwrite=True)
        ...
        assert n_tces_in_shard == len(fold_tce_tbl)
        writer.close()
...
# features to be added to the TFRecords from the TCE table
features_to_add = {
    'tce_period': {'dtype': 'float'},
    'tce_prad': {'dtype': 'float'},
    'tce_maxmes': {'dtype': 'float'},
    'wst_depth': {'dtype': 'float'},
    'tce_albedo_stat': {'dtype': 'float'},
    'tce_ptemp_stat': {'dtype': 'float'},
}
logger.info(f'Features to be added from the TCE table to the TFRecords: {features_to_add}')
...
jobs = [(shard_tbl_fp, dest_tfrec_dir, fold_i, src_tfrec_fps) for fold_i, shard_tbl_fp in enumerate(shard_tbls_fps)]
