...
# local
from models.exominer_joint_local_flux import ExoMinerJointLocalFlux
...
def predict_model(config, model_path, res_dir, logger=None):
    ...
    attn_scores = attn_scores_all['test']      # shape (B, H, Q, K)
    predictions = scores['test']               # shape (B,) or (B, 1)
    branch_names = ["stellar", "dv_tce_fit", "global_flux", "flux_trend", "local_centroid",
                    "flux_periodogram", "local_odd_even", "local_flux",
                    "local_weak_secondary", "local_unfolded_flux", "diff_img"]

    plot_attention_vs_prediction(attn_scores, predictions, branch_names,
                                title="Correlation between Attention and Prediction")
    plt.savefig(res_dir / "attn_vs_pred.png")
    plt.clf()
    plot_avg_attention_heatmap(attn_scores, branch_names)
    plt.savefig(res_dir / "attn_avg_heatmap.png")
    plt.clf()
...
def plot_attention_vs_prediction(attn_scores, predictions, branch_names, title="Attention vs Prediction Score"):
    """
    Plot correlation between attention paid to each branch and prediction score.

    Args:
        attn_scores: np.ndarray, shape (batch, heads, query_len, key_len)
        predictions: np.ndarray, shape (batch,) or (batch, 1)
        branch_names: list of branch names (length = query_len/key_len)
    """

    assert attn_scores.ndim == 4, "Expected attention shape (batch, heads, query_len, key_len)"
    assert predictions.ndim in (1, 2), "Expected predictions shape (batch,) or (batch, 1)"

    if predictions.ndim == 2:
        predictions = predictions.ravel()

    # Average attention scores over heads
    attn_avg_heads = attn_scores.mean(axis=1)

    # Sum attention to each key
    attention_to_each_branch = attn_avg_heads.sum(axis=1)

    # Correlate attention to prediction score (per branch)
    correlations = []
    for i in range(attention_to_each_branch.shape[1]):
        corr = np.corrcoef(attention_to_each_branch[:, i], predictions)[0, 1]
        correlations.append(corr)

    # Plot heatmap
    plt.figure(figsize=(8, 6))
    sns.heatmap(np.array(correlations)[None, :], xticklabels=branch_names, yticklabels=["corr"], cmap="coolwarm", center=0, annot=True)
    plt.title(title)
    plt.xlabel("Branch")
...
def plot_avg_attention_heatmap(attn_scores, branch_names, title="Average Attention Across Dataset"):
    """
    ...
    avg_attn = attn_scores.mean(axis=(0, 1))  # shape: (query_len, key_len)
