"""
Train models using a given configuration obtained on a hyperparameter optimization study in parallel.
"""
...
# else:
from src.estimator_util import InputFn, ModelFn, CNN1dModel
# from estimator_util import InputFn, ModelFn, CNN1dModel
# from src.eval_results import eval_model
from src.config import Config
# from config import Config


def draw_plots(res, save_path, eval_metric):
        """ Draw loss and evaluation metric plots.

        :param res: dict, with values for loss and metrics on the training, validation and test sets.
        :param save_path: str, filepath used to save the plots figure.
        :param eval_metric: str, metric minimized in the BOHB study.
        :return:
        """
        ...
        # loss plot
        ax[0].plot(res['epochs'], res['train']['loss'], label='Training')
        ax[0].plot(res['epochs'], res['val']['loss'], label='Validation', color='r')
        min_val_loss, ep_idx = np.min(res['val']['loss']), np.argmin(res['val']['loss'])
        ...
        ax[1].plot(res['epochs'], res['train'][eval_metric], label='Training')
        ax[1].plot(res['epochs'], res['val'][eval_metric], label='Validation', color='r')
        max_val_auc, ep_idx = np.max(res['val'][eval_metric]), np.argmax(res['val'][eval_metric])
        ax[1].scatter(res['epochs'][ep_idx], max_val_auc, c='r')
        # ax[1].scatter(res['epochs'][-1], res['test auc'], label='Test', c='k')
        ax[1].scatter(res['epochs'][ep_idx], res['test'][eval_metric][ep_idx], label='Test', c='k')
        ...
        ax[1].set_ylabel(eval_metric)
        ax[1].set_title('Evaluation Metric\nVal/Test %.4f/%.4f' % (max_val_auc, res['test'][eval_metric][ep_idx]))
        ...
        # precision, recall and auc plots
        f, ax = plt.subplots()
        ax.plot(res['epochs'], res['val']['precision'], label='Val Precision')
        ax.plot(res['epochs'], res['val']['recall'], label='Val Recall')
        ax.plot(res['epochs'], res['val']['roc auc'], label='Val ROC AUC')
        ax.plot(res['epochs'], res['val']['pr auc'], label='Val PR AUC')
        ax.plot(res['epochs'], res['test']['precision'], label='Test Precision')
        ax.plot(res['epochs'], res['test']['recall'], label='Test Recall')
        ax.plot(res['epochs'], res['test']['roc auc'], label='Test ROC AUC')
        ax.plot(res['epochs'], res['test']['pr auc'], label='Test PR AUC')
        ax.grid()
...
def run_main(config, save_path):
    ...
    res = {d: {m: [] for m in ['loss', 'pr auc', 'roc auc', 'precision', 'recall', 'accuracy']} for d
           in ['train', 'val', 'test']}
    res['epochs'] = []
    for epoch_i in range(1, config.n_epochs + 1):  # Train and evaluate the model for n_epochs
        ...
        for m in res['train']:
            res['train'][m].append(res_train[m])
    ...
    draw_plots(res, save_path=save_path, eval_metric='pr auc')
...
if __name__ == '__main__':
    ...
    save_path = '/home6/msaragoc/work_dir/HPO_Kepler_TESS/train_results/run_study_5/'
    ...
    res = hpres.logged_results_to_HBS_result('/home6/msaragoc/work_dir/HPO_Kepler_TESS/logs/study_5')
    ...
    print('Configuration chosen: ', best_config)
    ...
    run_main(Config(n_epochs=n_epochs, model_dir_path=save_path + 'models/', **config), save_path +
             'model%i' % (rank + 1))
